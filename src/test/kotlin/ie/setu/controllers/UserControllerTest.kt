...
class HealthTrackerControllerTest {
    ...
    inner class UpdateUsers {
        ...
        fun `updating a user when it exists, returns a 204 response`() {
            ...
            val addedResponse = addUser("Test", "test45i69766565@gmail.com")
            val addedUser : User = jsonToObject(addedResponse.body.toString())

            println("status ${addedUser.id}")
            val updatedName1="jAV"
            val updatedEmail1="JAYuV@gmail.com"
            //Act & Assert - update the email and name of the retrieved user and assert 204 is returned
            assertEquals(204, updateUser(addedUser.id, updatedName1, updatedEmail1).status)
            ...
            assertEquals(updatedName1, updatedUser.name)
            assertEquals(updatedEmail1, updatedUser.email)
